...
CONFIG = {
    "PROJECT_ROOT": "/scratch/user/haikookhandor/genai/code_nitai",
    "FUSION_FILE": "models/fusion_encoder_paper_v4.py",
    "DECODER_FILE": "models/lidar_decoder_paper_v4.py",
    "LOSSES_FILE":  "losses.py",


    "CHECKPOINT": "/scratch/user/haikookhandor/genai/code_nitai/paper_baseline/checkpoints/model_v4.pt",
    "EMBEDDING_PT": "/scratch/user/haikookhandor/genai/embeddings/s31/unit1/batch_0865_embeddings.pt",
    "EMB_INDEX": 0,

    "USE_AUTO_LIDAR": False,
    "LIDAR_DIR": "/scratch/user/haikookhandor/genai/datasets/s31/scenario31/unit1/lidar_data_renamed",
    "LIDAR_PLY":  "/scratch/user/haikookhandor/genai/datasets/s31/scenario31/unit1/lidar_data_renamed/lidar_data_6920.ply",
    ...
    "SAVE_PRED_PLY": True,
    "PRED_PLY_OUT": "/scratch/user/haikookhandor/genai/output/pred_pointcloud.ply",
}
# ===================================================================
...
MultimodalTransformerEncoder = _load_symbol(CONFIG["FUSION_FILE"],  "CrossModalTransformerEncoder")
LidarDecoderPaper           = _load_symbol(CONFIG["DECODER_FILE"], "LidarDecoderPaper")
...
def main():
    ...
    # Output directory
    out_dir = os.path.join(os.path.dirname(CONFIG["PRED_PLY_OUT"]), "plots")
    os.makedirs(out_dir, exist_ok=True)
    img_out_path = os.path.join(out_dir, "range_comparison.png")
    pc_out_path  = os.path.join(out_dir, "pointcloud_comparison.png")

    # Range images comparison
    ...
    fig.suptitle("GT vs Predicted LiDAR Range Images", fontsize=14)
    fig.savefig(img_out_path, dpi=300)
    plt.close(fig)
    print(f"[INFO] Saved range comparison plot → {img_out_path}")
    ...
    ax1.scatter(gt_plot[:,0], gt_plot[:,1], gt_plot[:,2], s=0.1, c='b')
    ax1.set_title("GT Point Cloud")
    ax1.set_xlabel("X (m)"); ax1.set_ylabel("Y (m)"); ax1.set_zlabel("Z (m)")

    ax2.scatter(pred_plot[:,0], pred_plot[:,1], pred_plot[:,2], s=0.1, c='orange')
    ...
    fig2.suptitle("GT vs Predicted LiDAR Point Clouds", fontsize=14)
    fig2.savefig(pc_out_path, dpi=300)
    plt.close(fig2)
    print(f"[INFO] Saved point cloud comparison plot → {pc_out_path}")
